...
static class SyntaxFilter {
    ...
    public static bool IsMultiResultHolder(this SyntaxNode sn) {
        if (sn is not StructDeclarationSyntax {
                Modifiers.Count: > 0,
                BaseList.Types.Count: > 0
            } sds
        ...
        foreach (var bt in sds.BaseList.Types) {
            ...
            if (gns is {
                    Identifier.Text: HolderTypeNames.MultiResultMarkerName,
                    TypeArgumentList.Arguments.Count: >= 2 and <= 8
                }
            ) {
                return true;
            }
