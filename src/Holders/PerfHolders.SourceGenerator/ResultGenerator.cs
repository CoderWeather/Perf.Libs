        ...
        var types = context.SyntaxProvider.CreateSyntaxProvider(
                ...
                static (context, ct) => {
                    ...
                    foreach (var i in symbol.Interfaces) {
                        if (i.FullPath() is Constants.ResultInterfaceFullName) {
                            if (marker != null) {
                                return default;
                            }
                    ...
                    var patternValues = new Dictionary<string, string?> {
                        ...
                        ["ResultName"] = symbol.Name,
                        ["ResultShort"] = symbol.MinimalName(),
                        ["ResultTypeofString"] = symbol.TypeArguments switch {
                            []               => symbol.Name,
                            [var t1]         => $"{symbol.Name}<{{typeof({t1.MinimalName()}).Name}}>",
                            [var t1, var t2] => $"{symbol.Name}<{{typeof({t1.MinimalName()}).Name}}, {{typeof({t2.MinimalName()}).Name}}>",
                        ...
                        ["OpenTypeArguments"] = symbol.TypeArguments switch {
                            [_]    => "<>",
                            [_, _] => "<,>",
                            _      => null
                        },
