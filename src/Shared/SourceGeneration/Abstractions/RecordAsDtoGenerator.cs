...
internal sealed class RecordAsDtoGenerator : IIncrementalGenerator {
    private const string RecordAsDtoAttributeFullName = "Perf.Abstractions.Attributes.RecordAsDtoAttribute";
    private const string AddAttributeToGeneratedDtoAttributeFullName = "Perf.Abstractions.Attributes.AddAttributeToGeneratedDto";
    private const string AddInterfacesToGeneratedDtoAttributeFullName = "Perf.Abstractions.Attributes.AddInterfacesToGeneratedDto";
    ...
    private static bool SyntaxFilter(SyntaxNode node, CancellationToken ct) {
        ...
        var attributeMarked = rec.AttributeLists
           .Any(x => x.Attributes.Any(y => y.ToString() is { } s
    ...
    private static string? GenerateSourceCode(string containingNamespace, RecPack[] types) {
        ...
        writer.WriteLines(
            "using Perf.Abstractions.Attributes;",
            $"using {containingNamespace};"
        );
        var nsFromProperties = types.SelectMany(x => x.Properties
               .SelectMany(p => new[] {
                    p.Type switch {
                        IArrayTypeSymbol at => at.ElementType.ContainingNamespace.ToDisplayString(),
                        _                   => p.Type.ContainingNamespace.ToDisplayString()
                    },
                    p.OriginalType switch {
                        IArrayTypeSymbol at => at.ElementType.ContainingNamespace.ToDisplayString(),
                        { } t               => t.ContainingNamespace.ToDisplayString(),
                        _                   => null
                    }
                }))
           .Where(x => x is not null)
           .Distinct();
        foreach (var ns in nsFromProperties) {
        ...
        foreach (var t in types) {
            foreach (var a in t.Attributes) {
                var formattedParams = a.Parameters.Select(x => x switch {
                    ...
                    foreach (var (pt, pn, ot) in t.Properties) {
                        if (ot is not null) {
                            if (ot.TypeKind is TypeKind.Array) {
                                ...
                                writer.WriteLine(
                                    $"this.{pn} = rec.{pn} != {nullWord} ? Perf.Utilities.ArrayUnsafe.MapCast<{ota.ElementType.GlobalName()}, {pta.ElementType.GlobalName()}>(rec.{pn}) : {nullWord};");
                            } else {
                    ...
                    foreach (var (pt, pn, ot) in t.Properties) {
                        if (ot is not null) {
                            if (ot.TypeKind is TypeKind.Array) {
                                ...
                                var nullWord = nullable ? "null" : "null!";
                                writer.WriteLine(
                                    $"{pn}: this.{pn} != {nullWord} ? Perf.Utilities.ArrayUnsafe.MapCast<{pta.ElementType.GlobalName()}, {ota.ElementType.GlobalName()}>(this.{pn}) : {nullWord}{(withComma-- > 0 ? ',' : null)}");
                            } else {
                                writer.WriteLine(
                                    $"{pn}: ({ot.GlobalName()}){pn}{(withComma-- > 0 ? ',' : null)}");
                            }
                        } else {
