...
internal static class Extensions {
    ...
    public static bool IsValueObject(this ITypeSymbol ts) {
        return ts.IsRecord
         && ts.IsValueType
         && ts.Interfaces.Any(x => x.Name is "IValueObject" or "IValidatableValueObject");
    }
    ...
    public static INamedTypeSymbol? TryGetValueObjectKeyType(this ITypeSymbol ts) {
        ...
        if (nullable) {
            voType = voType.WithNullableAnnotation(NullableAnnotation.Annotated);
        }
    ...
    public static T? TryGetArg<T>(this AttributeData data, int index = 0) {
        ...
        return args.IsDefaultOrEmpty ? default : args[index].As<T>();
    }
