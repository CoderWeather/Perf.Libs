...
internal sealed class MapperExtensionGenerator : IIncrementalGenerator {
    private const string RecordAsDtoAttribute = "Perf.Abstractions.Attributes.RecordAsDtoAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        var types = context.SyntaxProvider
           ...
           .Where(x => x.HasValue)
           .Select((nts, ct) => nts!.Value)
           .Collect();

        context.RegisterSourceOutput(types, CodeGeneration);
    }

    private static bool SyntaxFilter(SyntaxNode node, CancellationToken ct = default) {
        if (node is ClassDeclarationSyntax {
                TypeParameterList: null
            } cls) {
            var isSealed = cls.Modifiers.Any(SyntaxKind.SealedKeyword);
            var isAbstract = cls.Modifiers.Any(SyntaxKind.AbstractKeyword);
            var baseTypeCheck = cls.BaseList?.Types.Any(bt => bt.ToString() is "TypeAdapterConfig") is true;

            return isSealed && isAbstract is false && baseTypeCheck;
        }
    ...
    private static Mapper? SyntaxTransform(GeneratorSyntaxContext context, CancellationToken ct = default) {
        ...
        if (symbol.BaseType?.StrictEquals(typeAdapterConfigType) is false || symbol.InstanceConstructors.Length is not 1) {
            return null;
        }
        ...
        foreach (var m in configsList) {
            ...
            if (fromType.TryGetAttribute(CopyTypeMembersAttributeFullName) is { } fromCopy) {
            ...
            if (toType.TryGetAttribute(CopyTypeMembersAttributeFullName) is { } toCopy) {
