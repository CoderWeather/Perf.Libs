    ...
    public static void WriteNotificationBus(IndentedTextWriter writer, Assembly assembly, ImmutableArray<Handler> handlers) {
        ...
        using (NestedScope.Start(writer)) {
            writer.WriteLines(
                ...
                "	sp = serviceProvider;",
                $"	this.logger = sp.GetService<ILogger<{assembly.Mediator.Name}_NotificationBus>>();",
                "}"
            );
            ...
            using (NestedScope.Start(writer)) {
                ...
                if (notifications.Any()) {
                    ...
                    using (NestedScope.Start(writer)) {
                        ...
                        using (NestedScope.Start(writer)) {
                            ...
                            using (NestedScope.Start(writer)) {
                                foreach (var n in notifications) {
                                    ...
                                    using (NestedScope.Start(writer)) {
                                        writer.WriteLines(
                                            $"var handler = sp.GetService<{h.Type}>()!;",
                        ...
                        using (NestedScope.Start(writer)) {
                            writer.WriteLine(
                                "logger?.LogError(ex, \"Error publishing notification: {0}\", notification);");
                        }
                    }
                } else {
                    writer.WriteLines(
